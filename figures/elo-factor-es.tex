...
\tikz{ %
        ...
        \node[const, left=of r, xshift=-1cm] (r_name) {Resultado observado}; 
        ...
        \node[const, left=of p1, xshift=-0.5cm] (p_name) {Rendimiento oculto}; 
        ...
        \node[const, left=of s1, xshift=-1cm] (s_name) {Habilidad oculta}; 
        
        \node[factor, above=of s1] (fs1) {} ;
        \node[factor, above=of s2] (fs2) {} ;                        \node[const, right=of fs2] (ds) {\large $s \sim N(\widehat{\mu},\widehat{\sigma}^2)$};

        \node[const, right=of s2 , xshift=2.3cm,yshift=1.5cm] (prior) {\vast\}  $\hfrac{\text{\normalsize Distribuci\'on}}{\text{\normalsize de creencias}}$ };
        
        \node[const, right=of p2 , xshift=2.3cm,yshift=0.75cm] (prior) {\Vast\}  Modelo };

        \node[const, right=of p2 , xshift=2.3cm,yshift=-3cm] (prior) {\vast\}  Datos };
